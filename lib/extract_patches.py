...
def load_data(data_path_list_file):
    ...
    for i in range(len(img_list)):
        ...
        FOVs = np.expand_dims(fov, 0) if FOVs is None else np.concatenate(
            (FOVs, np.expand_dims(fov, 0)))

    assert(np.min(FOVs) == 0 and np.max(FOVs) == 255)
    assert((np.min(groundTruth) == 0 and (np.max(groundTruth) ==
           255 or np.max(groundTruth) == 1)))  # CHASE_DB1数据集GT图像为单通道二值（0和1）图像
    ...
    print('ori data shape < ori_imgs:{} GTs:{} FOVs:{}'.format(
        imgs.shape, groundTruth.shape, FOVs.shape))
    print("imgs pixel range %s-%s: " % (str(np.min(imgs)), str(np.max(imgs))))
    print("GTs pixel range %s-%s: " %
          (str(np.min(groundTruth)), str(np.max(groundTruth))))
...
def get_data_train(data_path_list, patch_height, patch_width, N_patches, inside_FOV):
    ...
    patches_imgs_train, patches_masks_train = extract_random(
        train_imgs, train_masks, train_FOVs, patch_height, patch_width, N_patches, inside_FOV)
...
def extract_random(full_imgs, full_masks, full_FOVs, patch_h, patch_w, N_patches, inside='not'):
    ...
    if (N_patches % full_imgs.shape[0] != 0):
        print(
            "\033[0;31mRecommended N_patches be set as a multiple of train img numbers\033[0m")
        N_patches = patch_per_img * full_imgs.shape[0]
    print("patches per image: " + str(patch_per_img),
          "  Total number of patches:", N_patches)
    patches = np.empty((N_patches, full_imgs.shape[1], patch_h, patch_w))
    patches_masks = np.empty(
        (N_patches, full_masks.shape[1], patch_h, patch_w), dtype=np.uint8)
        ...
        while k < patch_per_img:
            ...
            if inside == 'center' or inside == 'all':
                if not is_patch_inside_FOV(x_center, y_center, full_FOVs[i, 0], patch_h, patch_w, mode=inside):
                    continue
            patch = full_imgs[i, :, y_center-int(patch_h/2):y_center+int(
                patch_h/2), x_center-int(patch_w/2):x_center+int(patch_w/2)]
            patch_mask = full_masks[i, :, y_center-int(patch_h/2):y_center+int(
                patch_h/2), x_center-int(patch_w/2):x_center+int(patch_w/2)]
...
def is_patch_inside_FOV(x, y, fov_img, patch_h, patch_w, mode='center'):
    ...
    if mode == 'center':
        ...
        fov_patch = fov_img[y-int(patch_h/2):y+int(patch_h/2),
...
def get_data_test_overlap(test_data_path_list, patch_height, patch_width, stride_height, stride_width):
    ...
    test_imgs = paint_border_overlap(
        test_imgs, patch_height, patch_width, stride_height, stride_width)
    ...
    for i in range(full_imgs.shape[0]):  # loop over the full images
        for h in range((img_h-patch_h)//stride_h+1):
            for w in range((img_w-patch_w)//stride_w+1):
                patch = full_imgs[i, :, h *
                                  stride_h:(h*stride_h)+patch_h, w*stride_w:(w*stride_w)+patch_w]
                patches[iter_tot] = patch
                iter_tot += 1  # total
...
def pred_only_in_FOV(data_imgs, data_masks, FOVs):
    assert (len(data_imgs.shape) == 4 and len(
        data_masks.shape) == 4)  # 4D arrays
